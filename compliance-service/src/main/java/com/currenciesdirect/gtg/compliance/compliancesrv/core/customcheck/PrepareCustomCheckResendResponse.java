 ...
 */
public class PrepareCustomCheckResendResponse {
	
	/** The Constant LOGGER. */
	private static final Logger LOGGER = LoggerFactory.getLogger(PrepareCustomCheckResendResponse.class);
	...
	public Message<MessageContext> process(Message<MessageContext> message) {
		if (message.getHeaders().get(MessageContextHeaders.GATEWAY_OPERATION).equals( OperationEnum.CUSTOMCHECK_RESEND.name())) {

			MessageExchange exchange = message.getPayload().getGatewayMessageExchange();
			CustomCheckResendResponse response=exchange.getResponse(CustomCheckResendResponse.class);
			...
			LOGGER.warn("CustomCheckResendResponse Response : NO RESPONSE ");
		}
		return message;
	}
}
