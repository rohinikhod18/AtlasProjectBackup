...
public class CustomChecksTransformer extends AbstractCustomCheckTransformer{
	...
	public Message<MessageContext> transformRequest(Message<MessageContext> message) {
		...
		try {
			...
			//AT-2411
			Integer etailer = 3;
			if(etailer == account.getCustomerType())
				fundsInCreateReqeust.getTrade().setCustType(Constants.CFXETAILER);
			else
				fundsInCreateReqeust.getTrade().setCustType(fundsInCreateReqeust.getCustType());
			...
			ccExchange.addEventServiceLog(createEventServiceLogEntryWithStatus(eventId, ServiceTypeEnum.CUSTOM_CHECKS_SERVICE, 
					provider, account.getId(), account.getVersion() , EntityEnum.ACCOUNT.name(), token.getUserID(), 
					serviceResponse, serviceResponse, defaultStatus));			
	...
	public Message<MessageContext> transformResponse(Message<MessageContext> message) {
		...
		try{
			FundsInCreateRequest fundsInCreateReqeust=(FundsInCreateRequest) message.getPayload().getGatewayMessageExchange().getRequest();
			//AT-2411
			if(fundsInCreateReqeust.getTrade().getCustType().equalsIgnoreCase(Constants.CFXETAILER))
				fundsInCreateReqeust.getTrade().setCustType(Constants.CFX);
			...
			log = exchange.getEventServiceLog(ServiceTypeEnum.CUSTOM_CHECKS_SERVICE, EntityEnum.ACCOUNT.name(), account.getId());
			if(null != serviceResponse) {
				...
				updateEventServiceLogEntry(exchange.getEventServiceLog(ServiceTypeEnum.CUSTOM_CHECKS_SERVICE,
							EntityEnum.ACCOUNT.name(), account.getId()),serviceResponse, serviceResponse, serviceResponse.getOverallStatus());
