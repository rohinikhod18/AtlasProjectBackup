 ...
 *
 */
public class FundsInAggregator {
	private static final Logger LOG = LoggerFactory.getLogger(FundsInAggregator.class);
	...
	public Message<MessageContext> complieResponse(List<Message<MessageContext>> responses) {
		...
		try {
			// The received list will have 3 cloned Messages. And each will contain
			// and Exchange object for
			// KYC_SERVICE, FRAUGSTER_SERVICE, SANCTION_SERVICE, &
			// INTERNAL_RULE_SERVICE
			// INTERNAL_RULE_SERVICE can be taken from any of these Exhange object
			// however, we need to extract other exchange objects based on
			// ServiceTobeInvoked value

			// So for first msgs from list, keep only INTERNAL_RULE_SERVICE &
			// exchange for ServiceTobeInvoked
