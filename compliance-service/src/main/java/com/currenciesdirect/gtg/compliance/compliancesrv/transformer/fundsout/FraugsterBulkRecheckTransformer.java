...
public class FraugsterBulkRecheckTransformer extends AbstractTransformer{
	...
	public Message<MessageContext> transformRequest(Message<MessageContext> message) {
		try {
			...
			message.getPayload().removeMessageExchange(ServiceTypeEnum.FRAUGSTER_SERVICE);
	...
	private FraugsterPaymentsOutContactRequest transformPaymantOutRequest(FundsOutRequest fundsOutRequest)
		...
		requestData.setCustomerNo(trade.getTradeAccountNumber());
		...
		/**
		 * Changes made for to set unique TransactionID by concatenate PaymentOutID and TradeAccountNumber- Saylee
		 */
		...
		if (null != deviceInfo) {
			...
			//Added ScreenResolution instead of BrowserScreenResolution and DeviceResolution
		...
		String fraugsterSignupScore = (String) fundsOutRequest
				.getAdditionalAttribute("FraugsterSignupScore");
