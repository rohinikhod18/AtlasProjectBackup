...
/**
 * The Class PrepareBlacklistPayRefResendResponse.
 */
public class PrepareBlacklistPayRefResendResponse {

	
	/** The Constant LOGGER. */
	private static final Logger LOGGER = LoggerFactory.getLogger(PrepareBlacklistPayRefResendResponse.class);
	...
	public Message<MessageContext> process(Message<MessageContext> message) {
		try {
			if (message.getHeaders().get(MessageContextHeaders.GATEWAY_OPERATION)
					.equals(OperationEnum.PAYMENT_REFERENCE_RESEND.name())) {
				MessageExchange exchange = message.getPayload().getGatewayMessageExchange();
				PaymentReferenceResendResponse fResponse = exchange.getResponse(PaymentReferenceResendResponse.class);
				...
				LOGGER.warn("PaymentReferenceResendResponse Response : NO RESPONSE ");
			}
		} catch (Exception e) {
