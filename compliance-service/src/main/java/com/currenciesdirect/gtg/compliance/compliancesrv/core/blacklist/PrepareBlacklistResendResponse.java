...
/**
 * The Class PrepareBlacklistResendResponse.
 */
public class PrepareBlacklistResendResponse {

	
	/** The Constant LOGGER. */
	private static final Logger LOGGER = LoggerFactory.getLogger(PrepareBlacklistResendResponse.class);
	...
	public Message<MessageContext> process(Message<MessageContext> message) {
		try {
			if (message.getHeaders().get(MessageContextHeaders.GATEWAY_OPERATION)
					.equals(OperationEnum.BLACKLIST_RESEND.name())) {
				MessageExchange exchange = message.getPayload().getGatewayMessageExchange();
				BlacklistResendResponse fResponse = exchange.getResponse(BlacklistResendResponse.class);
				...
				LOGGER.warn("BlacklistResendResponse Response : NO RESPONSE ");
			}
		} catch (Exception e) {
