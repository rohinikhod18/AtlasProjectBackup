...
if (event.keyCode == 13) {
    document.getElementById("payOut_queue_Filter").click();
}
});
...
function applyPaymentOutSearchCriteria() {
	...
	$('#gifloaderforPayoutqueue').css('visibility','visible');
	currentPage = 1;
	setFilterApply(true);
...
function postPaymentOutQueueAjax(request,isFilterReq) {
	...
	$.ajax({
		url : '/compliance-portal/paymentOutQueue',
		type : 'POST',
		data : getJsonString(request),
		contentType : "application/json",
		success : function(data, status, jqXHR) {
			$('#gifloaderforPayoutqueue').css('visibility','hidden');
			...
			$('#payOutQueueForm,#pageCountDetails').css('display','block');
...
function getPaymentOutQueueDetails(paymentOutId,custType,anchor) {
	/** set false Origin is PaymentOut queue page*/
