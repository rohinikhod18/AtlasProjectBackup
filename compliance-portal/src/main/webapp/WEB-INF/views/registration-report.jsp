	...
	<tbody id="regQueueBody">
										...
										<c:forEach var="listValue"
											...
											<c:set var="regDetailsUrl" 
												...
												<td class="nowrap"><a onClick="getRegistrationQueueDetails(${listValue.contactId},'${listValue.type}',this)" href="#" >${listValue.type}</a></td>
...
<input type="hidden" id="source" value="report" name="source"/>
	...
	<a href="#" class="button--primary button--small" onclick="clearAndApplyFilter()"></a>
...
<section>
	...
	<p class="label filterInputHeader">Status</p>
		...
		<ul id="pillchoice-status-type" class="pill-choice--small">
			<li>
			
				<label class="pill-choice__choice--positive pill-choice_pill-choice_choice-on" for="chk-status-act">
				<input id="chk-status-act" name="status[]" type="checkbox" value="Active" class="filterInput"  />
				Active			</label></li>
			<li>
				<label class="pill-choice__choice--negative pill-choice_pill-choice_choice-on"  for="chk-status-inact">
				<input id="chk-status-inact" name="status[]" type="checkbox" value="Inactive" class="filterInput"  />
				Inactive		</label>
			</li>
			<li>
				<label class="pill-choice__choice--negative pill-choice_pill-choice_choice-on" for="chk-status-rej">
				<input id="chk-status-rej" name="status[]" type="checkbox" value="Rejected" class="filterInput"   />
				Rejected	</label>
			</li>
			</ul>
	...
	<div class="clickpanel__content--hidden">
		...
		<fieldset>
			...
			<ul class="multilist__options">
				...
				<c:forEach var="watchlistReason" items="${registrationQueueDto.watchList.watchlistData}"  varStatus="loop">
					
				<li>
				<label  for="chk-watchList-reason-${loop.index}">
					    <input name="watchListReason[]" class="filterInput" id="chk-watchList-reason-${loop.index}"
		...
		<%-- <fieldset>
			...
			<ul class="multilist__options">
              ...
              <c:forEach var="watchlistreason" items="${registrationQueueDto.watchListReason}"  varStatus="loop">
					
				<li>
				<label  for="chk-org-cduk-${loop.index}">
					    <input name="watchlistreason[]" class="filterInput" id="chk-org-cduk-${loop.index}"
...
<fieldset>
	...
	<li id="reg-service__status-filter" class="form__field-wrap ">
		...
		<legend class="label filterInputHeader">EID</legend>
						...
						<ul id="pillchoice-eid" class="pill-choice">
									...
									<i class="material-icons">close</i>			
											...
											<section class="section--actions">
											 <h2 id= "defaultMonth" style="visibility:hidden">Date defaults to last 6 months </h2>
												<input id="reg_report_Filter" type="button" class="button--primary button--small" value="GENERATE" onclick="applyRegQueueSearchCriteria()"/>
												...
												<object id="gifloaderforregreport" class="ajax-loader-lock-toggle" height="50" width="50" data="resources/img/ajax-loader.svg" preserveAspectRatio="xMidYMid slice" viewBox="0 0 16 16" type="image/svg+xml" style="visibility:hidden;"></object>
											</section>
										</form>
...
<input type="hidden" id="pageType" value="RegistrationReport"/>
</div>
...
<div id="downloadReportPopup" class="popupDiv" title="Alert">
	...
	<script type="text/javascript" src="resources/js/regReport.js"></script>
