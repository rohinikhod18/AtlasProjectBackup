...
public class SavedSearchControllerTest {

	@InjectMocks
	private SavedSearchController savedSearchController;

	@Mock
	ISavedSearchService iSavedSearchServiceResponse;
	...
	public boolean responceMocking() throws CompliancePortalException
	{
		SavedSearchRequest savedSearchRequest = savedSearchRequestMocking();
		UserProfile user = getUser();
		(when(iSavedSearchServiceResponse.savedSearch(user, savedSearchRequest))).thenReturn(false);
		(when(iSavedSearchServiceResponse.deleteSavedSearch(user, savedSearchRequest))).thenReturn(false);
		(when(iSavedSearchServiceResponse.updateSavedSearch(user, savedSearchRequest))).thenReturn(false);
		return false;
	}
	...
	public void testSavedSearch() throws CompliancePortalException {
		...
		UserProfile user = getUser();
		boolean saved;
		boolean expectedResult = responceMocking();
	    saved = savedSearchController.savedSearch(user, savedSearchRequest);
	    assertEquals(expectedResult, saved);
	}
	...
	public void testDeleteSavedSearch() throws CompliancePortalException {
		...
		boolean saved;
		boolean expectedResult = responceMocking();
	    saved = savedSearchController.deleteSavedSearch(user, savedSearchRequest);
		assertEquals(expectedResult, saved);
	}
	...
	public void testUpdateSavedSearch() throws CompliancePortalException {
		...
		boolean saved;
		boolean expectedResult = responceMocking();
	    saved = savedSearchController.updateSavedSearch(user, savedSearchRequest);
	    assertEquals(expectedResult, saved);
	}
