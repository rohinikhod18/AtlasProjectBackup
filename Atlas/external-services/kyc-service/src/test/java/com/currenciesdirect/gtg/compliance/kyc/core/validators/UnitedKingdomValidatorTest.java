...
public class UnitedKingdomValidatorTest {

	@InjectMocks
	UnitedKingdomValidator unitedKingdomValidator;
	...
	public void testValidateKYCRequest() {
		...
		try {
			FieldValidator validator = unitedKingdomValidator.validateKYCRequest(request);
	...
	public void testForValidateKYCRequest() {
		...
		FieldValidator validator1 = new FieldValidator();
		validator1.addError("can not be blank", "first_name, last_name, dob");

		try {
			FieldValidator validator = unitedKingdomValidator.validateKYCRequest(request);
			assertEquals(validator1.getErrors().get(0).getErrorMessage(),
					validator.getErrors().get(0).getErrorMessage());
		} catch (KYCException e) {
			System.out.println(e);
		}
	}
