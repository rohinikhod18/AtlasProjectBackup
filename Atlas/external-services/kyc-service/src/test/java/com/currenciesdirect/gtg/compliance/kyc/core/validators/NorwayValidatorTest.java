...
public class NorwayValidatorTest {
	
	@InjectMocks
	NorwayValidator norwayValidator;
	...
	public void testValidateKYCRequest() {
		...
		try {
			FieldValidator validator=norwayValidator.validateKYCRequest(request);
	...
	public void testForValidateKYCRequest() {
		...
		validator1.addError("can not be blank", "building_number");
		try {
			FieldValidator validator=norwayValidator.validateKYCRequest(request);
