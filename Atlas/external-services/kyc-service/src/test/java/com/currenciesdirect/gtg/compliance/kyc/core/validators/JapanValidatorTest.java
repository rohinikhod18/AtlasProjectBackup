...
public class JapanValidatorTest {
	
	@InjectMocks
	JapanValidator japanValidator;
	...
	public void testValidateKYCRequest() {
		...
		try {
			validator=japanValidator.validateKYCRequest(request);
	...
	public void testForValidateKYCRequest() {
		...
		validator1.addError("can not be blank", "last_name");
		try {
			FieldValidator validator=japanValidator.validateKYCRequest(request);
